...
# Only start model server when task_inference is local (user opted into local inference).
# In cloud mode, no model server = zero VRAM used. STT/voice won't work in cloud mode.
if [[ "$_TASK_PROVIDER" == "local" || "${KERNEL_EVO_FORCE_LOCAL:-0}" == "1" ]]; then
    if [[ $_SOCK_ALIVE -eq 0 ]]; then
    ...
    echo "[kernel-evolving] KERNEL_EVO_SKIP_MODEL_SERVER=1 — skipping model server (voice/STT will be unavailable)"
fi
