#!/usr/bin/env python3
"""
Sim 10 — Native Gemma 4 Tool-Calling Test.

Tests the native function-calling path (parse_response, tokenize=True,
sampling params, enable_thinking, tool_responses feedback loop) introduced in v1.10.4.
Same 5 tasks as sim9 for direct comparison.
...
Usage:
  python3 run_sim10.py              # all runs
  python3 run_sim10.py --run e2b    # single run
  python3 run_sim10.py --run e4b
  python3 run_sim10.py --run gpt
"""
...
OUT_BASE = Path.home() / "kernel-evo-notes" / "sim10"
RESULTS_DIR = Path(__file__).parent / "results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
TIMEOUT = 200  # doubled — local model + tool chaining need more time
...
def make_tasks(out_dir: Path):
    return [
        {
            "id": "T01_write_json",
            "message": f'Write this JSON to {out_dir}/config.json: {{"agent":"kernel-evo","version":"1.10.4","sim":"sim10"}}',
            "artifact": f"{out_dir}/config.json",
        },
        {
            "id": "T02_write_script",
            "message": f'Write a Python script to {out_dir}/hello.py with content: #!/usr/bin/env python3\nprint("kernel-evo sim10 local")',
            "artifact": f"{out_dir}/hello.py",
        },
        {
            "id": "T03_exec_and_save",
            "message": f'Run: echo "sim10 passed $(date)" and save the output to {out_dir}/timestamp.txt',
...
def run_task(task):
    ...
    try:
        result = _post("/message", {"message": task["message"], "chat_id": f"sim10-{task['id']}"})
...
def run_suite(run_key: str) -> dict:
    ...
    out_file = RESULTS_DIR / f"sim10_{run_key}_{ts_str}.json"
    payload = {
        "sim": "sim10", "run": run_key, "label": cfg["label"],
        "ts": datetime.now(timezone.utc).isoformat(),
